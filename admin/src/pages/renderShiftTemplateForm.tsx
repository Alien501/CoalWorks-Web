...
export const RenderShiftTemplateForm = () => {
  ...
  const fetchAndSetForm = async () => {
    setIsLoading(true);
    try {
      const res = await axios.get(`/api/data/shifttemplate/${params.id}`);
      console.log('see here')
      console.log(res.data.data.shiftTemplate)
      setFormData(res.data.data.shiftTemplate[0]);
    //   alert()
    } catch (error) {
      console.error(error);
